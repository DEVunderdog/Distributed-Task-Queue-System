...
func GetActiveJWTKey(ctx context.Context, isActive bool, store database.Store) ([]JWTKeyResponse, error){
	var jwtKeyList []JWTKeyResponse
	active := pgtype.Bool{
		Bool: isActive,
		Valid: true,
	}
	
	jwtStruct, err := store.GetLatestJWTKey(ctx, active)
	if err != nil {
		return nil, err
	}

	for _, jwtKeyData := range jwtStruct{
		data := JWTKeyResponse{
			ID: jwtKeyData.ID,
			PublicKey: jwtKeyData.PublicKey,
			PrivateKey: jwtKeyData.PrivateKey,
			ExpiresAt: jwtKeyData.ExpiresAt.Time,
		}

		jwtKeyList = append(jwtKeyList, data)
	}

	return jwtKeyList, nil
}
